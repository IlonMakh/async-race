...
export default class WinnersView {
    ...
    constructor() {
        ...
        this.checkDisable = async () => {
            const prev: HTMLButtonElement = document.querySelector('.winners_prev') as HTMLButtonElement;
            const next: HTMLButtonElement = document.querySelector('.winners_next') as HTMLButtonElement;
            const count = await WTOTALCOUNT();
            prev.disabled = WPAGE.number === 1;
            next.disabled = Math.ceil(count / WPAGE.limit) === WPAGE.number || count <= 10;
        };
    }
    ...
    drawWinnersBlock() {
        ...
        <div class='winners hide'>
            ...
            <div class='winners_page'>Page #${WPAGE.number}</div>
            <table cellspacing='0' class='winners_table'>
                <thead>
                    <tr><th>Number</th><th>Car</th><th>Name</th><th>Wins</th><th>Best time</th></tr>
                </thead>
                <tbody class = 'winners_content'></tbody>
            </table>
        </div>
        `;
        this.body.insertAdjacentHTML('beforeend', winnersHTML);
        window.addEventListener('DOMContentLoaded', async () => {
            const winnerTitle: HTMLElement = document.querySelector('.winners_title') as HTMLElement;
            winnerTitle.insertAdjacentHTML('beforeend', `Winners(${await WTOTALCOUNT()})`);
            this.drawAllWinners();
        });
    ...
    drawPagination() {
        ...
        (winners as HTMLElement).insertAdjacentHTML('beforeend', paginationHTML);
    }
}
